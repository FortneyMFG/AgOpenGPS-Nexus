...
{
    private Func<PoseSample> _poseProvider;
    private readonly float _bodyLength;
    private readonly float _bodyWidth;
    private bool _visible = true;

    public VehiclePassLayer(Func<PoseSample> poseProvider, float bodyLength = 6f, float bodyWidth = 3f)
    {
    ...
    /// <summary>
    /// Replaces the pose provider used when rendering the vehicle overlay.
    /// </summary>
    public void UpdatePoseProvider(Func<PoseSample> poseProvider)
    {
        _poseProvider = poseProvider ?? throw new ArgumentNullException(nameof(poseProvider));
    }
    ...
    public void Draw(GlInterface gl, in FrameCtx ctx)
    {
        ...
        var headingRadians = pose.HeadingRadians;
        var cos = Math.Cos(headingRadians);
        var sin = Math.Sin(headingRadians);

        var tip = new Double3(
            pose.EastMeters + (_bodyLength * 0.5 * cos),
            pose.NorthMeters + (_bodyLength * 0.5 * sin),
            0);
