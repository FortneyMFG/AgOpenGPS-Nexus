...
7. **Deterministic resolver** — Core interprets `requires.capabilities`, `requires.profiles`, and new relationship types (`peerOf`, `conflictsWith`, `replaces`, `extends`) alongside policy overlays to pick providers, quarantine conflicts, and publish an auditable decision log.【F:docs/SRS/sections/12_Extensibility_Plugins.md†L24-L32】【F:docs/plugins/nexus-plugin-dependency-map.md†L88-L205】
