...
[SRS CM5 section](../development/SRS/sections/5X_Hardware_IO_Device_Layer/54_CM5_Integrated_Controller.md)
...
1. Extract the Pumpkin Pi plugin zip into the Nexus install tree. The bundle
   ...
   expanded to `<install-roo../Plugins/org.agopengps.agio.pumpkinpi/<ver>/`.
2. Copy the example config and adjust HAL settings:
   ```bash
   sudo install -d /etc/aog
   sudo cp <install-roo../Plugins/org.agopengps.agio.pumpkinpi/<ver>/assets/config/pumpkin.yaml.example /etc/aog/pumpkin.yaml
   sudo editor /etc/aog/pumpkin.yaml
   ```
3. Keep the AgIO Bridge enabled for external adapters:
   ```bash
   sudo cp <install-roo../Plugins/org.agopengps.agio.pumpkinpi/<ver>/assets/config/bridge.yaml.example /etc/aog/bridge.yaml
   sudo systemctl enable --now agio-bridge
   ```
4. Deploy the systemd unit:
   ```bash
   sudo cp <install-roo../Plugins/org.agopengps.agio.pumpkinpi/<ver>/assets/systemd/pumpkin-pi.service /etc/systemd/system/
