...
operators receive timely updates even when complex reproductions are required. Pair this
guide with the [field feedback telemetry aggregator](field-feedback-telemetry.md) so every
escalation includes the latest anonymised trend snapshot and affected build data.
...
1. **Initial capture (0–4 hours)** — Regional support staff log requests in the ticketing
   ...
   snapshot for the affected machine/build. Reference the
   [installer/update channel guide](../../development/howto/installer-update-channels.md) to confirm the
   machine’s rollout ring before recommending mitigations.
...
- **Level 1 — Support engineering (≤ 24 hours)**: Validate reproduction steps, collect
  diagnostics, and apply known workarounds. Publish interim updates to the ticket every
  business day. Drop artefacts into the
  [community preview program](community-preview-program.md) channel when pilot fleets need
  early fixes for confirmation.
...
- **Level 3 — Cross-team task force (≤ 72 hours)**: For blocking fleet issues, the release
  manager convenes core/AGiO/plugin leads. They decide on hotfix scope, rollback, or feature
  flag adjustments, recording actions and ETA. Align the decision with the
  [mesh retention & privacy guide](../../development/howto/mesh-retention-privacy-operations-guide.md) when
  telemetry changes are required for verification.

## Status reporting

- **Daily digest** — Support coordinator circulates a digest summarising open escalations,
  severity, owners, and ETA using the latest aggregated telemetry counts for context. Include a
  link to the current `field-feedback.json` output so engineering can review raw metrics when
  needed.
...
- **Closure review** — When resolved, attach mitigation steps, firmware/app builds, and
  confirm with the dealer before marking closed. Feed findings back into the
  [community preview program](community-preview-program.md) to validate the fix with opt-in
  fleets before broad release.
