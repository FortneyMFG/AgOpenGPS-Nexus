...
2. **Drop / Install** – The operator (or an automated updater) copies the zip into either the bundled ../Plugins/` directory beside the Nexus binaries or into the user-scoped `%APPDATA%/Nexus/Plugins/inbox/`.
...
| Path | Purpose |
| --- | --- |
| ../Plugins/` | Bundled plugins that ship with Nexus. Installed under the application root. |
