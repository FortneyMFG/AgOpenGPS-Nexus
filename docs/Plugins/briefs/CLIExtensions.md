...
- **Sample adapter:** `Nexus.SamplePlugin.Cli` ships with calibrate/sniff verbs
  so teams can clone the patterns. Install it via `tools/ci/package-nx-cli.ps1`
  and copy the `sample-plugin` output into `~/.nexus/plugins` to test manifest
...
- **Adapter assemblies (`*.Cli.dll`)** implement `Nexus.Plugin.Cli.Abstractions`.
  ...
  installed locally.【F:docs/development/SRS/sections/9X_Frontends_Ops/93-ADR-054 - Nexus CLI Host and Plugin Verb Architecture.md†L20-L56】
- **Reflection services** let remote-only plugins expose verbs without shipping a
  ...
  services.【F:docs/development/SRS/sections/9X_Frontends_Ops/93_Command_Line_Interface.md†L24-L48】
...
service during development to confirm option metadata before writing adapters.
...
- **Output modes:** Support human-readable, `--json`, and `--ndjson` output to
  ...
  stable.【F:docs/development/SRS/sections/9X_Frontends_Ops/93_Command_Line_Interface.md†L31-L38】
...
- **Version gating:** Declare verb compatibility requirements in your plugin
  ...
  hints during `nx plugin list` and command execution.【F:docs/development/SRS/sections/9X_Frontends_Ops/93-ADR-054 - Nexus CLI Host and Plugin Verb Architecture.md†L36-L56】
...
- Ship adapter binaries as part of your plugin archive so single-file CLI builds
  ...
  the plugin manifest already uses for runtime assets.【F:docs/development/SRS/sections/9X_Frontends_Ops/94_Extensibility_Packaging_Updates.md#packaging-updates--catalog†L1-L60】 Run
