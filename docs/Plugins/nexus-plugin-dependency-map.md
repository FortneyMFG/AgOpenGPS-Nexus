...
```
  ...
  Core records the winning provider and decision inputs in the dependency resolution report so support teams can audit field rigs.【F:docs/development/SRS/sections/9X_Frontends_Ops/94_Extensibility_Packaging_Updates.md#packaging-updates--catalog†L7-L27】
...
- **Profiles:** `aog.mapping.v1/core`, `aog.mapping.v1/tiles` (CI attested) plus `mapping:vector` capability features for offline pyramid support so certified replacements can stand in for the official provider.【F:docs/development/SRS/appendices/schemas/plugin_manifest.schema.json†L153-L308】
...
- **Profiles:** Consumes `aog.mapping.v1/core@^1.2` and `mapping:vector` capability with `offlinePyramid` feature so attested third-party mapping plugins satisfy the hard dependency edges without aliasing.【F:docs/development/SRS/sections/9X_Frontends_Ops/94_Extensibility_Packaging_Updates.md†L24-L32】
