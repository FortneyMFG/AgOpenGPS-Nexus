...
- R-FE-070 (MUST, shared zone tool): Provide a Core-owned geometry editing toolbar (polygon, rectangle, brush, eraser, undo/redo) that binds to LayerEditService events so crop, genetics, profit, and risk plugins share consistent drawing affordances.【F:docs/ADR/ADR-044_ZoneDrawingFramework.md†L29-L74】
...
- R-FE-072 (SHOULD, collaborative awareness): Display active editors, mesh sync status, and edit history sourced from `LayerEditEvent` journals so operators avoid conflicting changes during multi-machine sessions.【F:docs/ADR/ADR-047_LiveTelemetryMesh.md†L33-L62】
...
- R-FE-052 (SHOULD, preset switcher & diff): Provide preset selectors, layout diff viewers, and snapshot/live-link indicators so operators understand the impact of applying presets or editing shared layouts.【F:docs/ADR/ADR-032-presets-and-layout-linking.md†L7-L35】
- R-FE-053 (SHOULD, task orchestration UI): Surface task progress, retries, and failures triggered by preset or layout changes so background preparation steps remain transparent and actionable.【F:docs/ADR/ADR-032-presets-and-layout-linking.md†L17-L34】
- R-FE-074 (SHOULD, analytics overlays): Offer quick toggles for crop type, genetics, yield, profit, risk, and weather overlays with legends and accessibility-compliant color ramps driven by plugin metadata.【F:docs/ADR/ADR-045_CropTypePlugin.md†L29-L71】【F:docs/ADR/ADR-049_YieldPlugin.md†L21-L52】【F:docs/ADR/ADR-052_FieldHealthPlugin.md†L21-L44】【F:docs/ADR/ADR-053_WeatherPlugin.md†L21-L49】
- R-FE-075 (SHOULD, weather timeline): Present session weather timelines with charting widgets, manual entry controls, and import status sourced from the Weather plugin; highlight deviations impacting spraying compliance.【F:docs/ADR/ADR-053_WeatherPlugin.md†L21-L49】
- R-FE-076 (SHOULD, report builder): Add a “Generate Report” flow with template selection, preview, and export actions wired to Report Builder APIs, showing dependency readiness per plugin section.【F:docs/ADR/ADR-051_ReportBuilder.md†L21-L52】
...
- R-FE-092 (MUST, session panel): Surface a session panel with environment snapshot, inputs, notes, and linked layers with provenance badges. Panel supports inline edits with autosave indicators and plugin badges (crop type, genetics, profit).【F:docs/SRS/sections/03_JobLifecycle.md†L45-L74】
...
- R-FE-033 (MUST, lifecycle hooks): Expose binding helpers for plugin UIs to subscribe to `onContextChanged`, `onSessionPause/Resume`, `onLayerStartEdit`, and mesh presence events without manual event-bus plumbing.【F:docs/SRS/sections/03_JobLifecycle.md†L18-L74】【F:docs/ADR/ADR-044_ZoneDrawingFramework.md†L29-L74】
