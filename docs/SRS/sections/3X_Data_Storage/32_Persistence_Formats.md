...
<<<<<<<< HEAD:docs/SRS/sections/3X_Data_Storage/32_Persistence_Formats.md
...
========
- R-DATA-051 (SHOULD, inventory provenance): Link `InventoryTransaction` entries to `SessionId`, `LayerId`, and `workOrderId` references so cost, compliance, and profit analytics can trace consumption back to field execution.【F:docs/SRS/sections/6X/62_Job_Lifecycle.md†L86-L109】【F:docs/ADR/ADR-050_CostProfitPlugin.md†L15-L40】
>>>>>>>> origin/develop:docs/SRS/sections/3X/32_Persistence_Formats.md
