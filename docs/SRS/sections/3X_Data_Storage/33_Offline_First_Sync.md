...
| Term | Definition |
|------|-------------|
| Offline-first Update | Package and workflow that installs without internet access, typically via USB or LAN share.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L6-L33】 |
| Staged Deployment | Sequential update of Core, UI, controllers with compatibility validation between stages.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L34-L94】 |
| Delta Package | Differential update artifact that transfers only changed bits to conserve bandwidth.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L64-L94】 |
| Rollback Checklist | Documented steps operators follow to revert to prior build with integrity verification.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L34-L120】 |
...
|----|-----------|-----------|---------|-----------------|-----------------------------|
| R-33000 | MUST | Distribution | Provide offline-capable packages (zip, Debian, AppImage, Docker) with documented install flows. | R-UPD-000, R-UPD-004 | Release pipelines publish signed artifacts with install guides; checksum verification required.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L6-L64】 |
| R-33001 | MUST | Rollback | Document rollback workflows per package type so rigs revert to known-good builds without re-imaging. | R-UPD-002, R-UPD-004 | Rollback checklist validated in QA; prior version retained locally. 【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L34-L94】 |
| R-33002 | SHOULD | Staged Updates | Allow staged Core/AgIO/controller updates with compatibility gates and validation prompts. | R-UPD-003, R-UPD-006 | Compatibility matrix enforced via tooling; failure blocks staged rollout.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L34-L120】 |
| R-33003 | COULD | Delta Delivery | Offer delta packages or background downloaders respecting limited connectivity. | R-UPD-005 | Optional service passes bandwidth utilization checks; falls back to full packages if validation fails.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L64-L94】 |
| R-33004 | MUST | Provenance & Integrity | Capture validation, checksum, and compatibility results before marking update successful. | R-UPD-006 | Update workflow logs stored for audit; failure triggers rollback instructions.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L34-L120】 |

### 33.5.2 Detailed Requirement Catalogue

| ID | Priority | Category | Summary | Source / Reference |
|----|-----------|-----------|---------|--------------------|
| R-UPD-000 | MUST | Distribution | Preserve zip-based distribution for offline installs.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L6-L24】 |
...
- Update tooling MUST run without admin/root requirements beyond those documented in package guides.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L6-L64】
- Packages MUST be installable offline with only artefacts included in the release bundle or on provided media.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L6-L64】
- Rollback instructions MUST preserve configuration and operator data, relying on retention policies from Section 34.【F:docs/SRS/sections/3X_Data_Storage/34_Backup_Retention_Archival.md†L6-L20】
...
- [A1] Operators can access at least one trusted device with internet to download updates before transferring offline.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L6-L64】
- [A2] Compatibility manifests are published alongside every release and versioned within the package bundle.【F:docs/SRS/sections/3X_Data_Storage/33_Offline_First_Sync.md†L34-L120】
- [A3] Backup tooling from Section 34 is in place before major updates, ensuring data can be restored if rollback fails.【F:docs/SRS/sections/3X_Data_Storage/34_Backup_Retention_Archival.md†L6-L20】
