...
- R-TH-030 (MUST, collaborative mesh telemetry): Stream presence, trail, coverage, and layer-delta topics over the Live Telemetry Mesh with QoS tiers, stale indicators, and ACL enforcement so multi-machine crews share context safely.【F:docs/SRS/sections/4X_Interprocess_Communications/42-ADR-047 - Live Telemetry Mesh.md†L21-L66】
- R-TH-031 (SHOULD, radio diagnostics): Expose RadioBridge counters (RSSI, retry rate, FEC usage, encryption status) in UI dashboards and logs to diagnose ELRS/LoRa links.【F:docs/SRS/sections/4X_Interprocess_Communications/42-ADR-048 - RadioBridge for ELRS LoRa Telemetry.md†L21-L56】
- R-TH-032 (SHOULD, edit history telemetry): Forward `LayerEditEvent` summaries to telemetry feeds so collaborative edits appear in health dashboards and reports.【F:docs/SRS/sections/7X_Mapping_Geospatial/72-ADR-044 - Zone Drawing Framework.md†L29-L74】
- R-TH-033 (SHOULD, weather logging health): Track weather auto-logging cadence, API import status, and sensor availability to alert operators when environmental data falls behind schedule.【F:docs/SRS/sections/7X_Mapping_Geospatial/72-ADR-053 - Weather & Environment Plugin.md†L33-L49】
...
- Linux service health must integrate with metrics/logging expectations before we can deploy headless rigs broadly.【F:docs/SRS/sections/2X_System_Architecture/21-O6%20-%20Linux%20Core%20service%20with%20remote%20frontends.md†L21-L44】【F:docs/SRS/sections/9X_Frontends_Ops/91_UI_Shell_Layout.md†L168-L176】
...
- **ADR-011 Mapping & visualization pipeline** will answer R-TH-020 and R-TH-021 by specifying render ordering, GPU textures, interpolation rules, and overview pyramids shared between UI and replay tooling.【F:docs/SRS/sections/2X_System_Architecture/21-ADR-900 - PoseStream, Layer, and Control Program Roadmap.md†L51-L57】
- **ADR-019 Provenance, audit, and QA** will connect telemetry overlays to quality scores and provenance tags defined in Section 11 so dashboards expose lineage along with visualization state.【F:docs/SRS/sections/2X_System_Architecture/21-ADR-900 - PoseStream, Layer, and Control Program Roadmap.md†L115-L121】
- **ADR-047 Live telemetry mesh** will define presence/trail QoS, stale detection, and ACL requirements aligned with R-TH-030 and R-TH-032.【F:docs/SRS/sections/2X_System_Architecture/21-ADR-900 - PoseStream, Layer, and Control Program Roadmap.md†L211-L232】
- **ADR-048 RadioBridge** covers radio framing, retry policies, and diagnostics referenced by R-TH-031.【F:docs/SRS/sections/2X_System_Architecture/21-ADR-900 - PoseStream, Layer, and Control Program Roadmap.md†L233-L248】
