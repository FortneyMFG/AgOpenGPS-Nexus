...
- R-TILE-000 (MUST, tile store): Maintain a vector/raster tile store with caching policies, compression, and eviction tuned for constrained storage; tiles must include provenance and schema version.【F:docs/SRS/sections/3X_Data_Storage/32_Persistence_Formats.md†L12-L156】【F:docs/SRS/options/9X/O-TEST-4_LayerReplayCI.md†L7-L44】
...
- R-TILE-003 (MUST, headless rendering): Offer server-side rasterization/export paths (PNG/GeoTIFF) driven by the same rendering pipeline for report generation and offline sync dashboards.【F:docs/SRS/options/9X/O-FRONT-6_RemoteClients.md†L12-L34】【F:docs/SRS/sections/9X_Frontends_Ops/94_Extensibility_Packaging_Updates.md†L59-L120】
- R-TILE-004 (SHOULD, GPU constraints): Validate pipelines on Windows, Linux desktop, and Raspberry Pi/CM5 GPUs; avoid features beyond OpenGL ES3 and provide fallbacks for software rendering.【F:SRS/Sections/1X_Platform_Foundations/11_OS_Support.md†L14-L45】
