...
Specify how Nexus captures environment configuration—profiles, secrets, feature flags, and per-device overrides—so deployments remain reproducible across desktops, CM5 controllers, and managed fleets.【F:docs/sections/6X_Core_Domain_Services/64_Telemetry_Health.md†L12-L128】【F:docs/sections/9X_Frontends_Ops/94_Extensibility_Packaging_Updates.md†L17-L211】
...
|--------------|-----------------|-----------------------|---------------------------|--------------------|
| Configuration Store | INI/flat files per UI host with manual overrides. | Hard to audit and share across rigs. | Versioned JSON/TOML profiles with schema validation. | 【F:docs/sections/6X_Core_Domain_Services/64_Telemetry_Health.md†L28-L96】 |
| Secrets | Stored inline or in plaintext config. | Risk of credential leakage on shared devices. | Encrypted secret stores with logical key references. | 【F:docs/sections/6X_Core_Domain_Services/64_Telemetry_Health.md†L94-L126】 |
| Feature Flags | Scattered toggles per executable. | No centralized governance or telemetry. | Shared flag registry consumed by UI, Core, plugins. | 【F:docs/sections/9X_Frontends_Ops/96_Quality_Engineering_Release.md†L149-L211】 |
...
|----|----------|----------|---------|----------------|-----------------------------|
| R-CONF-000 | MUST | Configuration Store | Use versioned JSON/TOML profiles with schema validation and provenance metadata. | Ops WG | Schema validation pipeline rejects invalid updates; history logged. 【F:docs/sections/6X_Core_Domain_Services/64_Telemetry_Health.md†L28-L96】【F:docs/sections/4X_Interprocess_Communications/41_Service_APIs_Contracts.md†L118-L158】 |
| R-CONF-001 | MUST | Secrets Management | Isolate credentials in encrypted stores with runtime leases referencing logical keys. | Security review | Secrets rotation tests verify expiry and renewal. 【F:docs/sections/6X_Core_Domain_Services/64_Telemetry_Health.md†L94-L126】 |
| R-CONF-002 | SHOULD | Profile Switching | Support operator-selectable profiles that swap transports, plugins, feature flags without manual edits. | UI WG | UI automation verifies profile swap <5 s and updates telemetry. 【F:docs/sections/9X_Frontends_Ops/91_UI_Shell_Layout.md†L70-L126】【F:docs/sections/9X_Frontends_Ops/94_Extensibility_Packaging_Updates.md†L17-L211】 |
...
|----|-------------|--------|---------------------|-------|
| RISK-24-1 | Secrets rotation may drift between Windows DPAPI and Linux keyrings. | High | Align providers with §22 deployment playbooks and automate lease validation per ADR-028.【F:docs/sections/2X_System_Architecture/21-ADR-028 - Nexus stack responsibilities & handoff boundaries.md†L78-L128】 | @ops-wg |
...
Adopt profile registry + overrides with schema validation and encrypted secret references, keeping flat file export for offline rigs while enabling managed fleet workflows.【F:docs/sections/9X_Frontends_Ops/94_Extensibility_Packaging_Updates.md†L17-L211】
