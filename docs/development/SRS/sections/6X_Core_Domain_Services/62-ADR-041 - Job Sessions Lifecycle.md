...
consistently.【F:docs/sections/6X_Core_Domain_Services/62-ADR-041 - Job Sessions Lifecycle.md†L11-L37】
...
* Each session carries stable identifiers (`sessionId`, `jobId`, optional `seasonId`, `farmId`, `fieldIds`),
  timestamps, operator roster, environmental measurements, equipment bundles, and plugin extensions.【F:docs/sections/6X_Core_Domain_Services/62-ADR-041 - Job Sessions Lifecycle.md†L37-L53】
...
* Sessions reference layers, telemetry logs, prescriptions, and attachments via stable IDs.【F:docs/sections/6X_Core_Domain_Services/62-ADR-041 - Job Sessions Lifecycle.md†L53-L86】
...
* Core publishes lifecycle events (`onSessionStart`, `onSessionPause`, `onSessionResume`, `onSessionEnd`,
  `onSessionMetadataChange`) buffered per plugin for deterministic acknowledgements.【F:docs/sections/6X_Core_Domain_Services/62-ADR-041 - Job Sessions Lifecycle.md†L86-L110】
...
* UI replaces “Run” with “Session”, offering drawers to control sessions, review notes, and capture metadata while
  Drive-In prompts trigger session start when geofence proximity matches templates.【F:docs/sections/6X_Core_Domain_Services/62-ADR-041 - Job Sessions Lifecycle.md†L110-L132】
...
* If plugins skip hooks or storage becomes read-only, Core annotates sessions with skipped reasons and switches to
  append-only journals while prompting operators.【F:docs/sections/6X_Core_Domain_Services/62-ADR-041 - Job Sessions Lifecycle.md†L132-L173】
...
for controllers, journaling, and automation coordination.【F:docs/sections/6X_Core_Domain_Services/62-ADR-041 - Job Sessions Lifecycle.md†L173-L189】
