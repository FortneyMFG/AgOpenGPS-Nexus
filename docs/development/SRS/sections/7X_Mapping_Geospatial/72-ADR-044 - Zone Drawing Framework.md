...
- **Layer registry alignment.** Editable layer declarations must reference the canonical registry entries introduced with the
  ...
  when sessions span multiple fields.【F:docs/development/SRS/sections/3X_Data_Storage/31-ADR-043 - Multi-Field Job Envelopes.md†L9-L112】【F:schemas/Layer.v1.json†L1-L120】
...
- **Collaborative audit trail.** Mesh share profiles must declare which edit events they replicate. ZoneService rejects
  ...
  program.【F:schemas/ShareProfile.v1.json†L1-L120】【F:docs/development/SRS/sections/9X_Frontends_Ops/94-ADR-031 - Official Plugin Bundle Dependency Governance.md†L17-L70】
...
- Editing sessions record the mounted `fieldIds[]` and union envelopes published by JobsService so replay, profit, and weather
  analytics can reconcile edits with the active job context without bespoke geometry joins.【F:docs/development/SRS/sections/6X_Core_Domain_Services/62-ADR-030 - Field job sessions and lifecycle services.md†L13-L96】
- `LayerEditService` broadcasts session lifecycle hooks (`onSessionStart`, `onSessionEnd`) and mesh presence metadata, allowing
  Profit, Yield, and Weather plugins to coalesce analytics windows with the exact edit ranges captured in journals.【F:docs/development/SRS/sections/4X_Interprocess_Communications/42-ADR-047 - Live Telemetry Mesh.md†L9-L70】【F:schemas/Session.v1.json†L1-L120】
- Collaborative edits over the Live Telemetry Mesh carry deterministic replay seeds so remote devices rebuild identical undo
  stacks even when RadioBridge transports introduce retries or resequencing during constrained connectivity.【F:docs/development/SRS/sections/4X_Interprocess_Communications/42-ADR-048 - RadioBridge for ELRS LoRa Telemetry.md†L9-L60】
...
- Fulfills R-DATA-040 and R-DATA-043 journal/schema expectations in §08 Data Model & Storage.【F:docs/development/SRS/sections/3X_Data_Storage/32_Persistence_Formats.md†L28-L33】
- Delivers shared toolbar and attribute panel experiences required by R-FE-070, R-FE-071, and R-FE-093 in §05 Frontends.【F:docs/development/SRS/sections/9X_Frontends_Ops/91_UI_Shell_Layout.md†L22-L36】
- Binds editing lifecycle hooks referenced by R-FE-033 and related job lifecycle events in §03 Job Lifecycle.【F:docs/development/SRS/sections/9X_Frontends_Ops/91_UI_Shell_Layout.md†L48】【F:docs/development/SRS/sections/6X_Core_Domain_Services/62_Job_Lifecycle.md†L21-L35】
