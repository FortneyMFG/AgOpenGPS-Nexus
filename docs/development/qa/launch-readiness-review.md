...
runbooks rather than recreating them ad hoc.【F:tools/ci/release.ps1†L4-L131】【F:README.md†L88-L99】【F:docs/development/howto/installer-update-channels.md†L11-L85】
...
- [ ] Verify SHA-256 hashes from the manifest against the staged Windows and Pi artefacts before promoting the build. Store the verification transcript alongside the manifest to prove the media is intact.【F:docs/development/howto/installer-update-channels.md†L22-L85】
- [ ] Confirm the Windows single-file executable, ZIP, and installer bundle plus the Raspberry Pi Debian package are present and match the packaging expectations documented for operators. Record download locations for each so support can guide late adopters.【F:docs/development/howto/installer-update-channels.md†L30-L117】
...
- [ ] Archive the signed checklist, manifest, QA artefacts, and support summaries in the release folder (`artifacts/release/<channel>/<timestamp>`) to preserve a single source of truth for the launch decision and future audits.【F:tools/ci/release.ps1†L69-L131】【F:docs/development/howto/installer-update-channels.md†L22-L85】
