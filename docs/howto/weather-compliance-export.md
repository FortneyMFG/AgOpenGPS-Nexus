...
and Regulatory plugin hooks.【F:docs/SRS/sections/7X_Mapping_Geospatial/72-ADR-053 - Weather & Environment Plugin.md†L9-L48】【F:docs/plugins/Regulatory.md†L1-L32】【F:docs/SRS/sections/9X_Frontends_Ops/91-ADR-051 - Report Builder & Export System.md†L9-L40】
...
2. **Overlay generation** — Confirm the Weather plugin is producing `weather.overlay` tiles aligned with the session timeline so
   spatial context is available in the export bundle.【F:docs/SRS/sections/7X_Mapping_Geospatial/72-ADR-053 - Weather & Environment Plugin.md†L21-L44】【F:docs/SRS/sections/3X_Data_Storage/32_Persistence_Formats.md†L54-L74】
...
4. **Report Builder template deployment** — Publish the `weather-compliance.v1` template that renders the summary PDF referenced
   below. Templates follow the Report Builder governance defined in ADR-051.【F:docs/SRS/sections/9X_Frontends_Ops/91-ADR-051 - Report Builder & Export System.md†L9-L41】
...
parity.【F:docs/SRS/sections/9X_Frontends_Ops/91-ADR-051 - Report Builder & Export System.md†L12-L40】 |
...
manifest.【F:docs/SRS/sections/7X_Mapping_Geospatial/72-ADR-053 - Weather & Environment Plugin.md†L21-L44】【F:docs/SRS/sections/3X_Data_Storage/32_Persistence_Formats.md†L54-L74】 |
...
- `overlayArtifacts[]` — References to exported overlay files plus metadata describing grid resolution, coordinate reference
  system, interpolation confidence, and checksums for each payload.【F:docs/SRS/sections/7X_Mapping_Geospatial/72-ADR-053 - Weather & Environment Plugin.md†L21-L44】
...
1. Trigger the Report Builder service with template `weather-compliance.v1`, scope=`session`, and options containing the desired
   ...
   renderers.【F:docs/SRS/sections/9X_Frontends_Ops/91-ADR-051 - Report Builder & Export System.md†L12-L40】
2. Request the Weather plugin’s export endpoint for the targeted session, providing the same export window. The plugin streams the
   overlay bundle and ingest log required for compliance packaging.【F:docs/SRS/sections/7X_Mapping_Geospatial/72-ADR-053 - Weather & Environment Plugin.md†L21-L44】【F:docs/plugins/Weather.md†L9-L28】
...
- **Retention and storage** — Follow the archival policies defined in the data lifecycle ADR to keep weather compliance packets
  accessible for the mandated window (e.g., 3–5 years) before transitioning to cold storage.【F:docs/SRS/sections/3X_Data_Storage/34-ADR-025 - Data lifecycle and retention policy.md†L1-L34】
- **Replays and QA** — Use the manifest’s checksums and timestamps when loading weather context into replay scenarios to guarantee
  deterministic analytics runs or audit reviews.【F:docs/SRS/sections/2X_System_Architecture/21-ADR-004 - Establish the composite simulation fabric (SimClock + SimBus).md†L31-L52】
