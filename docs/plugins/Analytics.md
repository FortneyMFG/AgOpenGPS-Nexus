...
- Handle `onLayerStartEdit`/`onFeatureCommit` events to recalculate analytics when operators adjust crop, genetics, risk, or profit zones via the shared editing toolchain.【F:docs/SRS/sections/7X_Mapping_Geospatial/72-ADR-044 - Zone Drawing Framework.md†L29-L74】
...
- Read `job.stats.fields[]` to present per-field summaries even when jobs span multiple fields. Aggregations should clearly
  delineate totals vs. per-field results.【F:docs/SRS/sections/3X_Data_Storage/31-ADR-043 - Multi-Field Job Envelopes.md†L47-L75】
