...
- Consume cost ledger entries stored in `CostRecord.v1.json` and yield layers from the Yield plugin to compute `profit.net` overlays registered in the Layer Registry.【F:schemas/CostRecord.v1.json†L1-L120】【F:docs/ADR/ADR-050_CostProfitPlugin.md†L21-L52】
...
- Expose UI forms and importers for manual entries (fuel, labor, inputs) and automated deductions sourced from layer provenance (e.g., rate controller logs).
...
- Support offline-first operation with local ledger storage, reconciling with shared stores when connectivity returns per ADR-030.【F:docs/ADR/ADR-030-field-job-sessions.md†L33-L86】
