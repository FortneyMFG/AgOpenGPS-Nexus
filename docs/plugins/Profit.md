...
- Support barcode/QR scanning for intake, transfers, and application reconciliation. When sessions close, the plugin posts `InventoryTransaction` entries keyed to `sessionId` and `workOrderId` to maintain traceability back to field work.【F:docs/SRS/sections/03_JobLifecycle.md†L86-L109】
