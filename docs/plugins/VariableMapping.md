...
=======
- **Planned vs. actual:** Publish planned layers under `vr.planned.*` IDs and reconcile them against as-applied `vr.actual.*` layers emitted by rate controllers. Provenance must include recipe hash, source layers, and session metadata.【F:docs/SRS/sections/7X/72_Mapping_Layers_Plugin.md†L65-L106】
>>>>>>> origin/develop
...
=======
- Subscribe to session lifecycle hooks to preload crop type, genetics, and profitability overlays before generating prescriptions. Session metadata supplies environment and operator context for audit logs.【F:docs/SRS/sections/6X/62_Job_Lifecycle.md†L18-L74】
>>>>>>> origin/develop
