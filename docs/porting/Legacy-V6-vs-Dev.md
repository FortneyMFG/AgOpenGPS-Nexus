...
- **Nexus direction:** Consolidates legacy inputs into the `MachineProfile` translator and validation harness, then exposes declarative profiles through gRPC/CLI tooling so vehicles, implements, and UI shells can be versioned independently while still supporting PGN bridges.【F:docs/porting/LegacyDataIngest.md†L24-L48】 It layers Presets/Layout services so operators apply live-linked or snapshot layouts alongside equipment presets without re-editing every profile.【F:docs/ADR/ADR-032-presets-and-layout-linking.md†L7-L35】
