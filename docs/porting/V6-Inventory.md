...
- Nexus port status: dynamic look-ahead scheduling and startup ramping now live in
  `AutoSteerLiteTuningProfile`/`AutoSteerLiteController` (see
  [`AutoSteerLite-Tuning.md`](AutoSteerLite-Tuning.md)).
...
- See [`LegacyDataIngest.md`](LegacyDataIngest.md) for the Nexus importer that consumes
  `TrackLines.txt`, `Boundary.txt`, and `Headland.txt` to materialise these structures.
